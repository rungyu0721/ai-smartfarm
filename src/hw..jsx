...
import { 
  LayoutDashboard, PlusCircle, PieChart, AlertTriangle, BrainCircuit, ArrowUpRight, 
  ArrowDownRight, Sprout, Sparkles, Lightbulb, LogOut, ChevronRight, Clock
} from 'lucide-react';
import { 
  Tooltip, ResponsiveContainer, Pie, Cell, Legend, PieChart as ChartContainer,
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'farm-ai-finance';

const apiKey = ""; // 請在此處填入您的 Gemini API Key
...
const App = () => {
  ...
  const [newEntry, setNewEntry] = useState({ title: '', amount: '', type: 'income', category: '一般收入' });
  ...
  const [aiAnalysis, setAiAnalysis] = useState({
    ...
    advice: [
      { title: "支出優化策略", content: "目前的農業支出佔比較高，建議檢查是否有節流空間。", icon: <ArrowDownRight className="text-red-500" />, priority: "緊急", color: "bg-red-50" },
      { title: "營收擴展建議", content: "民宿訂房收入為主要來源，建議可搭配農產品 DIY 活動提升客單價。", icon: <Sparkles className="text-blue-500" />, priority: "中等", color: "bg-blue-50" },
      { title: "能源節省計劃", content: "電費支出呈現上升趨勢，建議檢查溫室設備的能源效率。", icon: <Lightbulb className="text-emerald-500" />, priority: "長期", color: "bg-emerald-50" }
    ]
  });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const combined = [...cloudData, ...initialData].sort((a, b) => new Date(b.date) - new Date(a.date));
  ...
  const runFullAIAnalysis = async () => {
    ...
    try {
      ...
      setAiAnalysis({
        ...
        advice: result.advice.map(a => ({
          ...a,
          icon: a.priority === '緊急' ? <AlertTriangle className="text-red-500" /> : <Lightbulb className="text-emerald-500" />,
          color: a.priority === '緊急' ? "bg-red-50" : (a.priority === '中等' ? "bg-blue-50" : "bg-emerald-50")
        }))
      });
    } catch (e) {
      console.error(e);
    ...
    <div className="flex h-screen bg-[#F8FAFC] text-slate-800 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        <header className="bg-white px-10 py-6 border-b border-slate-100 flex justify-between items-center sticky top-0 z-20">
          ...
          <button onClick={() => setShowModal(true)} className="bg-[#10B981] hover:bg-[#059669] text-white px-6 py-2.5 rounded-full flex items-center gap-2 shadow-lg shadow-emerald-500/20 transition-all font-bold text-sm">
        ...
        <div className="p-10 space-y-8">
          {activeTab === 'dashboard' && (
            <div className="space-y-8">
              ...
              {/* 圓餅圖 */}
              <div className="bg-white p-8 rounded-[32px] shadow-sm border border-slate-100">
                <h3 className="text-sm font-bold mb-8 flex items-center gap-2 text-slate-500 uppercase tracking-widest"><PieChart size={16} /> 收支類別佔比分析</h3>
          ...
          <div className="bg-white rounded-[40px] w-full max-w-lg p-10 shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleSave} className="space-y-6">
              <div className="grid grid-cols-2 gap-3">
                 <button type="button" onClick={() => setNewEntry({...newEntry, type: 'income'})} className={`py-4 rounded-2xl font-bold text-xs transition-all border-2 ${newEntry.type === 'income' ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-slate-50 border-transparent text-slate-400'}`}>收入項目</button>
                 <button type="button" onClick={() => setNewEntry({...newEntry, type: 'expense'})} className={`py-4 rounded-2xl font-bold text-xs transition-all border-2 ${newEntry.type === 'expense' ? 'bg-slate-50 border-slate-200 text-slate-700' : 'bg-slate-50 border-transparent text-slate-400'}`}>支出項目</button>
              </div>
